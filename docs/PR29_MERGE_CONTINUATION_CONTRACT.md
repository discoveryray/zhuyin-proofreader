...
- `corrections` 必須恰六筆連續，原五輪固定提交鏈不可改，第六筆 N→C。`current` 為 D/C/C tree 與 clean-tree 實證；`continuation` 保留重建標籤、14 缺件及 KF-01～KF-07、RC-01～RC-04 至少全部已知 finding。KF-07 要求新 C 的遠端雙版本 CI，故僅在 C 本機、遠端仍 N 時可標 `pending_pr_ci`，且不得填 verification ref 或 verified head；其餘 finding 此時須有 C 的具體 verification ref 與 `verified_head=C`。C 的 PR CI 通過後 KF-07 必須以該 run/attempt 的原始 `pr_ci.evidence_ref`、`verified_head=C` 轉為 verified，才能進 R2、解除草稿或合併。缺失報告未知內容仍未知。
